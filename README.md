...
```bash
tar -xzf bird-v0.1.3-aarch64-apple-darwin.tar.gz
install -m 755 bird-v0.1.3-aarch64-apple-darwin/bin/bird /usr/local/bin/bird
```

Linux x64 uses the corresponding archive name:

```bash
tar -xzf bird-v0.1.3-x86_64-unknown-linux-gnu.tar.gz
install -m 755 bird-v0.1.3-x86_64-unknown-linux-gnu/bin/bird /usr/local/bin/bird
```
